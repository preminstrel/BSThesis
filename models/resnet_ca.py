"""
    Add channel attention (CBAM https://arxiv.org/abs/1807.06521) to ResNet backbone
"""
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        f, channel_atten = self.featuremaps(x)
        #v = self.global_avgpool(f)
        return f#, channel_atten
...
def resnet18_ca(pretrained=True, **kwargs):
...
def resnet34_ca(pretrained=True, **kwargs):
...
def resnet50_ca(pretrained=True, **kwargs):
...
def resnet101_ca(pretrained=True, **kwargs):
...
def resnet152_ca(pretrained=True, **kwargs):
